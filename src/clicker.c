...
static TextLayer *science_text_layer;
static TextLayer *shop_text_layer;

static int num_science = NUM_COOKIE_DEFAULT;
...
static void update_text() {
  ...
  snprintf(body_text, sizeof(body_text), "%u", num_science);
  text_layer_set_text(science_text_layer, body_text);
}
...
void menu_select_callback(MenuLayer *menu_layer, MenuIndex *cell_index, void *data)
{
    ...
    if(which == 0) {
      if(num_science >=75000){
        num_clicks=num_clicks*2;
        num_science=num_science-75000;
        update_text();
      }
    }
    ...
    if(which == 1) {
      if(num_science >=100){
        ...
        num_science=num_science-100;
        update_text();
      }
    }
    if(which == 2) {
      if(num_science >=500){
        num_clicks=num_clicks+7;
        num_science=num_science-500;
        update_text();
      }
    }
    if(which == 3) {
      if(num_science >=5000){
        num_clicks=num_clicks+75;
        num_science=num_science-5000;
        update_text();
      }
    }
    if(which == 4) {
      if(num_science >=20000){
        num_clicks=num_clicks+400;
        num_science=num_science-20000;
        update_text();
      }
    } 
	if(which == 5) {
      if(num_science >=40000){
        num_clicks=num_clicks+900;
        num_science=num_science-40000;
        update_text();
      }
    } 
}
...
static void increment1_click_handler(ClickRecognizerRef recognizer, void *context) {
  num_science=num_science+num_clicks;
  if (num_science > 80000) {
    text_layer_set_font(science_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_28));
  }
  update_text();
...
void select_long_click_handler(ClickRecognizerRef recognizer, void *context) {
  num_science=0;
  num_clicks=1;
  text_layer_set_font(science_text_layer, fonts_get_system_font(FONT_KEY_BITHAM_34_MEDIUM_NUMBERS));
  update_text();
}
...
static void window_load(Window *me) {
  ...
  //Tetlayer details for science
   science_text_layer = text_layer_create(GRect(1, 1, width, 60));
  text_layer_set_font(science_text_layer, fonts_get_system_font(FONT_KEY_BITHAM_34_MEDIUM_NUMBERS));
  text_layer_set_background_color(science_text_layer, GColorClear);
  layer_add_child(layer, text_layer_get_layer(science_text_layer));
...
static void window_unload(Window *window2) {
  ...
  text_layer_destroy(science_text_layer);
...
static void init(void){
  ...
  num_science = persist_exists(NUM_COOKIE_PKEY) ? persist_read_int(NUM_COOKIE_PKEY) : NUM_COOKIE_DEFAULT;
...
static void deinit(void) {
  ...
  persist_write_int(NUM_COOKIE_PKEY, num_science);
